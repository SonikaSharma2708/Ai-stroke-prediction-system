...
def predict():
    ...
    if not model:
        ...
        return jsonify({"status": "success", "risk_percentage": mock_risk})
    ...
    try:
        ...
        work_type = work_map.get(request.form.get('work_type'), 0)
        
        res_map = {'Urban': 1, 'Rural': 0}
        residence_type = res_map.get(request.form.get('Residence_type'), 1)
        
        glucose = float(request.form.get('avg_glucose_level', 100))
        bmi = float(request.form.get('bmi', 25))
        
        smoke_map = {'never smoked': 0, 'formerly smoked': 1, 'smokes': 2, 'Unknown': 3}
        smoking_status = smoke_map.get(request.form.get('smoking_status'), 0)
        ...
        return jsonify({"status": "success", "risk_percentage": round(prob, 1)})
    except Exception as e:
...
def download_report():
    return render_template('report.html',
                           patient_id=request.form.get('pdf_id', 'Anonymous Record'),
                           age=request.form.get('pdf_age', 'N/A'),
                           gender=request.form.get('pdf_gender', 'N/A'),
                           glucose=request.form.get('pdf_glucose', 'N/A'),
                           bmi=request.form.get('pdf_bmi', 'N/A'),
                           hypertension=request.form.get('pdf_hyper', 'No'),
                           risk=float(request.form.get('pdf_risk', 0.0)))
